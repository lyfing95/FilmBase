			...
			<div class="booking-details">
				...
				<ul class="book-right">
				  ...
				  <c:if test="${loginUser != null}">
				      <button class="checkout-button" onclick="return submitTo()">确认选座</button>       <!-- </a>  -->
				   </c:if>
				  <c:if test="${loginUser == null}">
		       ...
		       //实现选座
						click: function () {   
							if (this.status() == 'available') { 
								...
								var limit = sc.find('selected').length;
								if(limit>3){
									alert("一次最多购买4张票");
									return 'available';
								}else{
				...
				//已售座位
				       sc.get(<%=session.getAttribute("selectedSeat")%>).status('unavailable');  
				});   
				//总价
				...
				function submitTo(){
					...
					/* var existSeatid=${wachplan.seatid}; */
					var check = $("#selected-seats").val(); 
					if(cart == "" || cart ==null){
				    	alert("请选择座位号！");
				        return false;
				    }else{
				    $.ajax({
						...
						type:"get",
						data:{"total":total,"count":count,"seats":seats,"userid":userid,"watchid":watchid},/* "existSeatid":existSeatid */
						dataType:"text",
						success:function(data){
						  var thisOrdersid ;
							location.href='film/ticketConfirm.do'; 
							/* if(data != null){
							    location.href='film/ticketConfirm.do'; 
							}else{
								alert("请选择座位号！");
